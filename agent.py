"""
╔══════════════════════════════════════════════════════════════╗
║           ИИ-АГЕНТ «ФЕНИКС» v0.3  —  Max Performance         ║
...
LLM_MODEL     = "llama3"
LLM_TIMEOUT   = 90          # секунды ожидания ответа
...
DEFAULT_PROFILE = {
    "name":        "Феникс",
    "version":     "0.3",   # ← v0.2 (твой) + v3.0 (enhanced) = v0.3
...
"""
        ...
        if parallel_calls:
            ...
            for tr in results:
                ...
                _log(f"         [{tr['tool']}] → {out_preview[:130]}"
                     f"{'…' if len(out_preview) > 130 else ''}")
        else:
            _log(f"[Шаг {step}] ⏳ Размышление без инструментов")
...
def main():
    ...
    while True:
        ...
        # ── служебные команды ──────────────────────────────────
        if cmd == "exit":
            print(f"👋 До свидания! Я — {profile.get('name')}."); break
        ...
        # ── основной вызов агента (стриминг финального ответа) ─
        print()
        # шаги рассуждения — без стриминга (JSON нельзя стримить)
        answer = agent(user_input, memory, rag, profile)
        # вывод уже произошёл внутри agent(); печатаем красиво
