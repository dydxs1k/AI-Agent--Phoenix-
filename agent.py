...
Доступные инструменты:
  ...
  • run_python  — РЕАЛЬНО выполнить Python-код в отдельном процессе и вернуть stdout.
                  input: {"code": "print(2+2)"}
                  ⚠️  ОБЯЗАТЕЛЕН когда нужно: посчитать, построить, обработать данные,
                  написать скрипт. НИКОГДА не симулируй выполнение кода в тексте!
  • remember    — сохранить факт навсегда.    input: {"text": "важный факт"}
  • none        — ответить напрямую (только для вопросов БЕЗ кода и вычислений).
"""
...
{{
  "thought":        "<рассуждение — НЕ пиши сюда код, только мысли>",
...
• Если пользователь просит посчитать сложное выражение — используй calculator или run_python.
...
# Глобальный флаг видимости потока мыслей (True = показывать)
_SHOW_THOUGHTS: bool = True

def _log(msg: str, kind: str = "grey"):
    # "answer" показываем всегда; остальное — только если _SHOW_THOUGHTS
    if not _SHOW_THOUGHTS and kind != "answer":
        return
    colors = {
        "thought": _C["yellow"],
        "tools":   _C["blue"],
...
def _print_help():
  ...
  \033[1;33m📋 Команды:\033[0m
    ...
    \033[1mмысли\033[0m                 — показать/скрыть поток мыслей агента
...
def main():
    ...
    while True:
        ...
        if cmd in ('мысли', 'thoughts', 'verbose', 'тихо', 'quiet'):
            global _SHOW_THOUGHTS
            _SHOW_THOUGHTS = not _SHOW_THOUGHTS
            state = "\033[32mВКЛЮЧЁН\033[0m" if _SHOW_THOUGHTS else "\033[90mОТКЛЮЧЁН\033[0m"
            print(f"  🧠 Поток мыслей: {state}")
            continue
